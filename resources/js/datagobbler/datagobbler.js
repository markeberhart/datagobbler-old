!function() {
    ...
    datagobbler.getData = function(_url,_cb,_lcb){
        d3.json(_url, function(error,data) {
            if(error){
                ...
                datagobbler.dataOptions = data.dataOptions;
                datagobbler.dataOptions.dates = datagobbler.setDefaultDates(data.dataOptions.defaultDates);
    ...
    datagobbler.setDataOptions = function(){
        ...
        this.dataOptions.defaultDates['idateStart'] = this.getCommonTime(this.dataOptions.dates.dateStart);
        this.dataOptions.defaultDates['idateEnd'] = this.getCommonTime(this.dataOptions.dates.dateEnd);
    ...
    datagobbler.setDefaultDates = function(args){
        ...
        if(typeof args.dateEnd === 'string'){ 
            ...
            _t = new moment(); 
            _obj.dateEnd = new moment().utc().add(args.dateEnd,'days');
            _obj.dateStart = new moment().utc().add(args.dateEnd,'days');
            _obj.defaultUIDateStart = new moment().utc().add(args.dateStart,'days');
            _obj.defaultUIDateEnd = new moment().utc().add(args.dateEnd,'days');
        }
        
        if(typeof args.dateStart === 'string'){
            _obj.dateStart = new moment(args.dateStart).utc(); 
        }else{ 
            _obj.dateStart.utc().add(args.dateStart,'days');
        }
        
        if(typeof args.defaultUIDateEnd === 'string'){ 
            _obj.defaultUIDateEnd = new moment(args.defaultUIDateEnd).utc();
        }else{
            _obj.defaultUIDateEnd.utc().add(args.defaultUIDateEnd,'days');
        }

        if(typeof args.defaultUIDateStart === 'string'){ 
            _obj.defaultUIDateStart = new moment(args.defaultUIDateStart).utc();
        }else{
            _obj.defaultUIDateStart.utc().add(args.defaultUIDateStart,'days');
        }
    ...
    datagobbler.createDataServiceUrls = function() { 
        for(dl in datagobbler.dataLayers){ 
            ...
            datagobbler.dataLayers[dl].apiInfo.url = _url; datagobbler.dataLayers[dl]['loadingStatus'] = {'mustDownload':false,'proxyLayer':false,'pendingDownload':false,'downloadCompleted':false,'downloadError':false,'isFiltered':false};
    ...
    datagobbler.showLoadingStatus = function(evt){
        for(s in evt.loadingStatusObject){
            ...
            var _obj = {"layer":s,"title":datagobbler.dataLayers[s].title,"loadingStatus":evt.loadingStatusObject[s]};
    ...
    datagobbler.downloadDataZIP = function(layer){
        console.log("datagobbler.downloadDataZIP called");
        var _data = datagobbler.dataLayers[layer].apiInfo.data;
        var _arr = [];
        var _filetype = datagobbler.dataLayers[dl].apiInfo.fileType;
        var _loadingStatus = datagobbler.dataLayers[layer].loadingStatus;
    ...
    //TODO: Check to see if filtering is deleting items and fix!
     datagobbler.decodeDataCSV = function(layer){
        ...
        var _geomType   = datagobbler.dataLayers[layer].apiInfo.ifNotSpatialDataType.geometryType;
        var _latStr     = datagobbler.dataLayers[layer].apiInfo.ifNotSpatialDataType.fieldToUseForLatitude;
        var _lonStr     = datagobbler.dataLayers[layer].apiInfo.ifNotSpatialDataType.fieldToUseForLongitude;
        var _features = [];
    ...
    datagobbler.filterSuccess = function(layer){
        //console.log(layer,"datagobbler.DownloadSuccess");
        datagobbler.dataLayers[layer].loadingStatus.isFiltered = true;
        document.dispatchEvent(datagobbler.loader);
        //datagobbler.layerOkToFilter(layer);
        //datagobbler.numOfDataFilesToFilter++;
         //console.log("-------------------------------");
    ...
    */
        ...
        for(k in _dataArray){
            ...
            for(f in _features){
                ...
                if(_notDeleted){
                    ...
                    var _incDate = datagobbler.dataLayers[layer].apiInfo.dateInfo.overwriteDate.toLowerCase();
                    ...
                    if(_incDate=="true"){
                        ...
                        if(_features[f].properties[_dateField]){
                            ...
                            _time = datagobbler.dataLayers[layer].apiInfo.dateInfo.dateToUseIfOverwritingDate;
                        }
    ...
    datagobbler.addToBackgroundData = function(layer,feature) { 
        ...
        feature['id'] = datagobbler.numRecords;
